        ...
        <h2>
          ...
          @auth
            @if (auth()->user()->username === $sharedData['username'])
                <a href='/change-avatar' class='btn btn-sm btn-secondary'>
                  Change Avatar
                </a>
            @endif
          
            @if (!$sharedData['isFollowed'] AND 
                auth()->user()->username !== $sharedData['username'])
            ...
            @if ($sharedData['isFollowed'] AND auth()->user()->username !== $sharedData['username'])
              <form class="ml-2 d-inline" 
              action="/remove-follow/{{$sharedData['username']}}" method="POST">
                @csrf
                <button class="btn btn-danger btn-sm">Stop Following <i class="fas fa-user-times"></i></button>
              </form>
            @endif
